...
async function checkDeviceStatus() {
  try {
    ...
    if (data.success && data.data) {
      ...
      if (deviceInfo.serviceError) {
        ...
        statusBadge.innerHTML =
          '<i class="material-symbols-rounded me-1">error</i>خدمة الأجهزة غير متاحة';
        ...
        statusAlert.innerHTML = `
          <div class="alert alert-danger d-flex align-items-center" role="alert">
            ...
            <div>
              <strong>تحذير:</strong> 
              ${
                deviceInfo.serviceDown
                  ? 'خدمة أجهزة البصمة غير متاحة'
                  : deviceInfo.timeout
                  ? 'انتهت مهلة الاتصال بأجهزة البصمة'
        ...
        statusBadge.innerHTML =
          '<i class="material-symbols-rounded me-1">fingerprint</i>لا توجد أجهزة متاحة';
      ...
      statusBadge.className = 'badge bg-danger';
      statusBadge.innerHTML =
        '<i class="material-symbols-rounded me-1">error</i>خطأ في فحص الأجهزة';
    ...
    statusBadge.className = 'badge bg-danger';
    statusBadge.innerHTML =
      '<i class="material-symbols-rounded me-1">error</i>خطأ في الاتصال';
...
async function attendStudent(event) {
  ...
  console.log('📤 Sending attendance request:', {
    ...
    selectedGroupText:
      groupSelection.options[groupSelection.selectedIndex]?.text,
  });
  try {
    ...
    if (response.ok) {
      ...
      if (
        response.status === 403 &&
        responseData.message === 'هذا الطالب محظور من المركز'
      ) {
        Swal.fire({
          icon: 'error',
          title: 'طالب محظور',
          html: `
              ...
              <p><strong>سبب الحظر:</strong> ${
                responseData.blockReason || 'غير محدد'
              }</p>
              <p><strong>تاريخ الحظر:</strong> ${
                responseData.blockedAt
                  ? new Date(responseData.blockedAt).toLocaleDateString()
                  : 'غير محدد'
              }</p>
            `,
        ...
        response.status === 403 &&
        responseData.message === 'هذا الطالب غير مسجل في المجموعة المختارة'
      ) {
...
const getStudents = async () => {
  try {
    ...
    const response = await fetch(
      `/employee/get-attended-students?groupId=${groupId}`
    );
    const responseData = await response.json();
    if (!response.ok) {
  ...
  students.forEach((student) => {
    ...
    tr.innerHTML = `
            ...
            <td class="text-center">${
              student.student.studentPhoneNumber || 'N/A'
            }</td>
            <td class="text-center">${
              student.student.studentParentPhone || 'N/A'
            }</td>
            <td class="text-center">${
            ...
            <td class="text-center">
              <button class="btn btn-danger btn-sm delete" data-student-id="${
                student.student._id || ''
              }">Delete</button>
              ${
                !monthlyPaymentPaid
                  ? `<button class="btn btn-success btn-sm pay-monthly" data-student-id="${
                      student.student._id || ''
                    }">دفع شهري</button>`
                  : ''
              }
            ...
            <td class="text-center">${
              student.addedBy ? student.addedBy.employeeName : 'N/A'
            }</td>
          `;
...
async function deleteStudent(studentId, groupId) {
  try {
    spinner.classList.remove('d-none');
    const response = await fetch(
      `/employee/delete-attend-student/${studentId}?groupId=${groupId}`,
      {
        method: 'DELETE',
      }
    );
...
endAttendanceBtn.addEventListener('click', async () => {
  try {
    ...
    // Show loading state
    endAttendanceBtn.innerHTML =
      '<i class="material-symbols-rounded text-sm">hourglass_empty</i>&nbsp;&nbsp;جاري البدء...';
    ...
    if (response.ok && responseData.success) {
      ...
      if (responseData.details && responseData.details.status === 'started') {
        const { totalAbsent, estimatedTimeMinutes, groupName } =
          ...
          html: `
                        ...
                        ${
                          responseData.details
                            ...
                            <p>عدد الطلاب الغائبين: ${
                              responseData.details.totalAbsent
                            }</p>
                            ${
                              responseData.details.messagesSent !== undefined
                                ? `<p>الرسائل المُرسلة: ${responseData.details.messagesSent}</p>`
                                : ''
                            }
                            ${
                              responseData.details.errors > 0
                                ? `<p style="color: orange;">عدد الأخطاء: ${responseData.details.errors}</p>`
                                : ''
                            }
                        `
                            : ''
      ...
      const errorMessage =
        responseData.message || 'حدث خطأ أثناء إرسال الرسائل';
    ...
    // Reset button state
    endAttendanceBtn.innerHTML =
      '<i class="material-symbols-rounded text-sm">check_circle</i>&nbsp;&nbsp;نهاية الحضور';
...
document.addEventListener('DOMContentLoaded', function () {
  const quickNotificationType = document.getElementById(
    'quickNotificationType'
  );
  const quickNotificationFilter = document.getElementById(
    'quickNotificationFilter'
  );
  const quickNotificationMessage = document.getElementById(
    'quickNotificationMessage'
  );
  const sendQuickNotificationsBtn = document.getElementById(
    'sendQuickNotificationsBtn'
  );
  const quickNotificationStatus = document.getElementById(
    'quickNotificationStatus'
  );
  ...
  async function sendQuickNotifications() {
    ...
    try {
      ...
      switch (filter) {
        ...
        case 'perSession':
          endpoint =
            '/employee/api/students-with-balances?paymentType=perSession';
          break;
        case 'all':
        default:
          endpoint = '/employee/api/students-with-balances';
    ...
    sendQuickNotificationsBtn.innerHTML =
      '<i class="material-symbols-rounded text-sm me-1">hourglass_empty</i>جاري الإرسال...';
    ...
    try {
      ...
      sendQuickNotificationsBtn.innerHTML =
        '<i class="material-symbols-rounded text-sm me-1">send</i>إرسال إشعارات سريعة';
    }
  }
